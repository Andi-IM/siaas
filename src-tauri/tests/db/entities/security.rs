...
use app_lib::db::entities::{majors, batches, semesters, subjects, curriculum_subjects, students};
...
async fn test_foreign_key_constraints() {
    ...
    students::ActiveModel {
        id: Set(student_id.clone()), major_id: Set(major_id.clone()), full_name: Set("Jane".to_string()),
        nis: Set("54321".to_string()), nisn: Set("1122334455".to_string()), created_at: Set(now.clone()), updated_at: Set(now.clone()),
        ..Default::default()
    }.insert(&db).await.unwrap();
...
async fn test_sql_injection_prevention() {
    ...
    let insert_result = students::ActiveModel {
        id: Set(malicious_id.to_string()), major_id: Set(major_id.clone()), full_name: Set("H".to_string()),
        nis: Set(malicious_nis.to_string()), nisn: Set("999".to_string()), created_at: Set(now.clone()), updated_at: Set(now.clone()),
        ..Default::default()
    }.insert(&db).await;
