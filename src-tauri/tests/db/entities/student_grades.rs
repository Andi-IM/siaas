...
use app_lib::db::entities::{
    batches, curriculum_subjects, majors, semesters, student_grades, students, subjects,
};
...
async fn test_student_grades_crud() {
    ...
    students::ActiveModel {
        ...
        major_id: Set(major_id.clone()),
        full_name: Set("J".to_string()),
        nis: Set("1".to_string()),
        nisn: Set("1".to_string()),
        created_at: Set(now.clone()),
        updated_at: Set(now.clone()),
        ..Default::default()
    }
    ...
    // Read
    let grade = student_grades::Entity::find_by_id(grade_id.clone())
        .one(&db)
        .await
        .unwrap()
        .unwrap();
    ...
    let updated = student_grades::Entity::find_by_id(grade_id.clone())
        .one(&db)
        .await
        .unwrap()
        .unwrap();
    ...
    // Delete
    student_grades::Entity::delete_by_id(grade_id.clone())
        .exec(&db)
        .await
        .unwrap();
    let deleted = student_grades::Entity::find_by_id(grade_id.clone())
        .one(&db)
        .await
        .unwrap();
