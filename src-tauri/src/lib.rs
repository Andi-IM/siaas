use crate::db::paths::{AppPathProvider, DbPathProvider};
...
async fn get_app_logs(app_handle: tauri::AppHandle) -> Result<String, String> {
    ...
    let file_content = std::fs::read_to_string(&log_path).map_err(|e| e.to_string())?;
...
async fn reset_database(
    ...
    db_conn.close_by_ref().await.map_err(|e| e.to_string())?;
    ...
    if !deleted && db_path.exists() {
        return Err(
            "Gagal menghapus berkas basis data lama karena masih terkunci oleh sistem.".to_string(),
        );
    }
    ...
    let new_conn = db::establish_connection(&db_path)
        .await
...
async fn export_database(
    ...
    std::fs::copy(&db_path, std::path::Path::new(&path)).map_err(|e| e.to_string())?;
...
async fn import_database(
    ...
    std::fs::copy(src_file, &db_path).map_err(|e| e.to_string())?;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let db_path = provider
                .get_db_path(app.handle())
