...
pub async fn delete_program_core<C: ConnectionTrait>(db: &C, id: &str) -> Result<bool, AppError> {
...
pub async fn get_majors_core<C: ConnectionTrait>(db: &C) -> Result<Vec<majors::Model>, AppError> {
...
pub async fn delete_major_core<C: ConnectionTrait>(db: &C, id: &str) -> Result<bool, AppError> {
...
pub async fn get_batches_core<C: ConnectionTrait>(db: &C) -> Result<Vec<batches::Model>, AppError> {
...
pub async fn get_subjects_core<C: ConnectionTrait>(
    ...
    let list = subjects::Entity::find().all(db).await?;
...
pub async fn delete_subject_core<C: ConnectionTrait>(db: &C, id: &str) -> Result<bool, AppError> {
...
pub async fn delete_student_core<C: ConnectionTrait>(db: &C, nis: &str) -> Result<bool, AppError> {
...
pub async fn get_subjects_by_major_core<C: ConnectionTrait>(
    ...
    let mut result_map: std::collections::HashMap<String, MataPelajaranData> =
...
pub async fn assign_subject_to_semesters_core(
    ...
    let batch = batches::Entity::find().one(db).await?;
...
pub async fn upsert_student_grade_core<C: ConnectionTrait>(
    ...
    if !(0.0..=100.0).contains(&grade) {
        return Err(AppError::Validation(
            "Nilai harus berada di antara 0 dan 100".to_string(),
        ));
    }
...
pub async fn get_grades_by_filter_core<C: ConnectionTrait>(
    ...
    let students_list = students::Entity::find().all(db).await?;
    ...
    for g in grades {
        ...
        let mapping = curriculum_subjects::Entity::find_by_id(g.curriculum_subject_id.clone())
            ...
            .ok_or_else(|| AppError::NotFound {
                entity: "CurriculumSubject",
                field: "id",
                value: g.curriculum_subject_id.clone(),
            })?;
        ...
        let subject = subjects::Entity::find_by_id(mapping.subject_id.clone())
            ...
            .ok_or_else(|| AppError::NotFound {
                entity: "Subject",
                field: "id",
                value: mapping.subject_id.clone(),
            })?;
...
pub async fn import_grades_from_excel_core(
    ...
    if range.height() < 8 {
        return Err(AppError::Excel(
            "Berkas Excel tidak valid (minimal harus 8 baris)".to_string(),
        ));
    }
    ...
    let major_info = match major_val {
        ...
        _ => {
            return Err(AppError::Excel(
                "Format sel A3 harus berupa teks (KONSENTRASI KEAHLIAN : NAMA)".to_string(),
            ))
        }
    };
    ...
    let all_majors = majors::Entity::find().all(&txn).await?;

    let major = all_majors
        .into_iter()
        .find(|m| m.name.eq_ignore_ascii_case(major_name))
        .ok_or_else(|| AppError::NotFound {
            entity: "Major",
            field: "name",
            value: major_name.to_string(),
        })?;
    ...
    let subjects_list = subjects::Entity::find().all(&txn).await?;

    let semesters_list = semesters::Entity::find().all(&txn).await?;
    ...
    for col in 6..width {
        ...
        if let Some(val) = range.get_value((4, col as u32)) {
            ...
            if !sem_name.is_empty() {
                if sem_name.contains("SEMESTER 1") {
                    current_semester = 1;
                } else if sem_name.contains("SEMESTER 2") {
                    current_semester = 2;
                } else if sem_name.contains("SEMESTER 3") {
                    current_semester = 3;
                } else if sem_name.contains("SEMESTER 4") {
                    current_semester = 4;
                } else if sem_name.contains("SMT 5") || sem_name.contains("SEMESTER 5") {
                    current_semester = 5;
                } else if sem_name.contains("SEMESTER 6") {
                    current_semester = 6;
                } else if sem_name.contains("UKK") {
                    current_semester = 0;
                } // Skip UKK column
            }
        }
...
fn populate_excel(
    ...
    worksheet.write_with_format(
        1,
        0,
        format!("PROGRAM STUDI : {}", program_name.to_uppercase()),
        &title_format,
    )?;
    worksheet.write_with_format(
        2,
        0,
        format!("KONSENTRASI KEAHLIAN : {}", major_name.to_uppercase()),
        &title_format,
    )?;
    ...
    for (s_idx, s) in students_list.iter().enumerate() {
        ...
        worksheet.write_with_format(
            r as u32,
            2,
            s.place_of_birth.as_deref().unwrap_or(""),
            &row_student_format,
        )?;
        worksheet.write_with_format(
            r as u32,
            3,
            s.date_of_birth.as_deref().unwrap_or(""),
            &row_date_format,
        )?;
        ...
        for (c_idx, col_spec) in columns_to_export.iter().enumerate() {
            ...
            if let Some(g) = grades
                .iter()
                .find(|g| g.student_id == s.id && g.curriculum_subject_id == col_spec.mapping_id)
            {
...
pub async fn export_grades_to_excel_core<C: ConnectionTrait>(
    ...
    let subjects_list = subjects::Entity::find().all(db).await?;

    let semesters_list = semesters::Entity::find().all(db).await?;
    ...
    columns_to_export.sort_by(|a, b| match a.semester_sequence.cmp(&b.semester_sequence) {
