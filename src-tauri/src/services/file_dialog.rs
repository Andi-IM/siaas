...
pub trait FileDialogService: Send + Sync {
    ...
    fn save_file(&self, title: &str, filters: Vec<(String, Vec<String>)>, default_name: Option<String>) -> Option<SelectedFile>;
}
...
impl FileDialogService for NativeFileDialog {
    ...
    fn save_file(&self, _title: &str, filters: Vec<(String, Vec<String>)>, default_name: Option<String>) -> Option<SelectedFile> {
        let mut dialog = rfd::FileDialog::new();
        for (name, exts) in filters {
            let exts_refs: Vec<&str> = exts.iter().map(|s| s.as_str()).collect();
            dialog = dialog.add_filter(name, &exts_refs);
...
impl FileDialogService for MockFileDialog {
    fn pick_file(&self, _title: &str, _filters: Vec<(String, Vec<String>)>) -> Option<SelectedFile> {
    ...
    fn save_file(&self, _title: &str, _filters: Vec<(String, Vec<String>)>, _default_name: Option<String>) -> Option<SelectedFile> {
        self.mock_path.as_ref().map(|p| SelectedFile {
            name: p.file_name().unwrap_or_default().to_string_lossy().to_string(),
            path: p.to_string_lossy().to_string(),
        })
