...
export default function StudentTranscriptView({ nis }: { nis: string }) {
  ...
  const pendampingCategories: PendampingCategory[] = React.useMemo(() => {
    ...
    subjects.forEach(s => {
      subjectGradeMap[s.id] = {
        name: s.nama,
        sequence: s.sequence,
        transcriptGroup: s.transcriptGroup ?? "UMUM",
        grades: new Map(),
      };
    });
  ...
  const handleExportPdf = async () => {
    try {
      const message = await invoke<string>("export_transcript_pdf", { nis });
      console.log(message);
    } catch (e) {
    ...
    <div className="list-page">
      ...
      <div className="list-action-bar no-print">
        ...
        <div className="list-action-bar__right">
          ...
          <button className="btn btn--secondary" onClick={handleExportPdf} style={{ gap: 8 }}>
            <Download size={18} />
            Simpan PDF
          </button>
