...
export default function CurriculumPage() {
  ...
  const refreshConcentrations = async (pId: string) => {
    try {
      ...
      if (cons.length > 0) {
        if (!selectedKonsentrasiId) setSelectedKonsentrasiId(cons[0].id);
      } else {
  ...
  const openModal = (type: Exclude<ModalType, null>, id: string | null = null) => {
    if (type === "program") {
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginTop: 16 }}>
                ...
                <div className="form-field">
                  ...
                  <select className="form-input" value={subjectForm.transcriptGroup} onChange={e => setSubjectForm({ ...subjectForm, transcriptGroup: e.target.value as TranscriptGroup })}>
                    ...
                    <option value="KIK">KIK (Kreativitas, Inovasi, dan Kewirausahaan)</option>
